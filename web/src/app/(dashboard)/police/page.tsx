...
export default function PoliceDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'anomaly-detection' | 'digital-ids' | 'alerts' | 'locations' | 'missing-persons' | 'efir-generation' | 'investigation-tools'>('overview');
    ...
    <RoleGuard requiredRoles={['super_admin', 'police_admin', 'operator']}>
      <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100">
                Digital ID management, alert response, and missing person investigations
              </p>
            </div>
            <div className="text-right">
              <div className="text-sm text-blue-200">On Duty Officers</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <StatCard
            title="Digital IDs"
            value={stats.totalDigitalIDs.toString()}
            icon={<Fingerprint className="h-6 w-6" />}
            subtitle={`${stats.verifiedIDs} verified`}
            color="blue"
          />
          <StatCard
            title="Active Alerts"
        ...
        <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            ...
            { id: 'anomaly-detection', label: 'AI Anomaly Detection', icon: Brain },
            { id: 'digital-ids', label: 'Digital IDs', icon: Fingerprint },
            { id: 'alerts', label: 'Alert History', icon: AlertTriangle },
            { id: 'locations', label: 'Locations', icon: MapPin },
            { id: 'missing-persons', label: 'Missing Persons', icon: Users },
            { id: 'efir-generation', label: 'E-FIR Generation', icon: FileText },
            { id: 'investigation-tools', label: 'Investigation Tools', icon: Search }
          ].map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="min-h-[600px]">
          ...
          {activeTab === 'investigation-tools' && <InvestigationTools />}
        </div>
      </div>
    </RoleGuard>
  );
          ...
          {mockAlertHistory.filter(alert => alert.severity === 'critical' || alert.severity === 'high').map((alert) => (
            <div key={alert.id} className="flex items-start justify-between p-3 bg-red-50 rounded-lg">
              <div className="flex-1">
                <div className="font-medium text-gray-900">{alert.touristName}</div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-3">
          {mockMissingPersons.map((mp) => (
            <div key={mp.id} className="flex items-start justify-between p-3 bg-orange-50 rounded-lg">
              <div className="flex-1">
                <div className="font-medium text-gray-900">{mp.touristName}</div>
              ...
              <div className="text-right">
                ...
                {mp.firNumber && (
                  <div className="text-xs text-gray-500 mt-1">{mp.firNumber}</div>
                )}
              </div>
            </div>
          ))}
      ...
      <Card className="p-6">
        ...
        <div className="space-y-3">
          {mockLastKnownLocations.map((location) => (
            <div key={location.touristId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div>
                <div className="font-medium text-gray-900">{location.touristName}</div>
                <div className="text-sm text-gray-600">{location.location.zone}</div>
              </div>
              <div className="text-right">
                <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${
                  location.isEmergency ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                }`}>
        ...
        {filteredIDs.map((id) => (
          <Card key={id.id} className="p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <Badge 
                variant={
                  id.verificationStatus === 'verified' ? 'success' : 
                  id.verificationStatus === 'pending' ? 'warning' : 'destructive'
                }
              >
        ...
        {mockLastKnownLocations.map((location) => (
          <Card key={location.touristId} className="p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <Badge variant={location.isEmergency ? 'destructive' : 'success'}>
                {location.isEmergency ? 'Emergency' : 'Safe'}
              </Badge>
            </div>
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right">
                ...
                <Badge variant={
                  ...
                  mp.status === 'investigating' ? 'warning' :
                  mp.status === 'found' ? 'success' :
                  'secondary'
                } className="ml-2">
                  {mp.status}
                </Badge>
